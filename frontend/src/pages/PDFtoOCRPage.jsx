...
const PDFtoOCRPage = () => {
    ...
    const handleExtract = async () => {
        ...
        try {
            // --- Step 1: Convert PDF to Images via Backend ---
            setProcessStatus('Step 1/2: Converting PDF to images...');
            const formData = new FormData();
            formData.append('file', file);

            const convertResponse = await fetch('http://localhost:3001/api/pdf/pdf-for-ocr', {
                method: 'POST',
                body: formData,
            });

            if (!convertResponse.ok) {
                throw new Error('Failed to convert PDF to images on the server.');
            }
            ...
            // --- Step 2: Run OCR on each image on the Frontend ---
            let combinedText = '';
            const worker = await createWorker('eng');

            for (let i = 0; i < imageFiles.length; i++) {
                const imageName = imageFiles[i];
                const imageUrl = `http://localhost:3001/outputs/${imageName}`;
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-10">
                <h2 className="text-4xl font-bold text-primary">PDF to Text (OCR)</h2>
                <p className="text-lg text-muted-foreground mt-2">Extract text from scanned (image-based) or regular PDFs.</p>
            </div>
                ...
                {!file ? <FileUploader onDrop={handleDrop} accept={{ 'application/pdf': ['.pdf'] }} /> : (
                    <div>
                        ...
                        <div className="text-center mt-8">
                            ...
                            {isProcessing && <p className="text-sm text-muted-foreground mt-2">{processStatus}</p>}
                        </div>
                    </div>
                )}
                {extractedText && (
                    <div className="mt-8">
                        ...
                        <textarea readOnly value={extractedText} rows="12" className="w-full p-4 bg-secondary border border-border rounded-md text-secondary-foreground" />
                        <div className="flex items-center justify-center space-x-4 mt-6"><button onClick={handleCopy} className="btn btn-secondary"><FiCopy className="inline mr-2" /> Copy</button></div>
                    </div>
                )}
